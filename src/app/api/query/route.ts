...
export async function POST(req: NextRequest) {
  try {
    ...
    // 5. Run the query
    const results = await usersCollection.find(mongoFilter).toArray();
    console.log("DB query results:", results.length);
...
function fixDateExpressions(filter: any): any {
  // Recursive function to traverse and fix date expressions
  ...
  for (const key in filter) {
    if (key === "$expr" && typeof filter[key] === "object") {
      for (const op of exprOps) {
        if (Array.isArray(filter[key][op])) {
          const args = filter[key][op];
          for (let i = 0; i < args.length; i++) {
            if (args[i].$date) {
              args[i] = new Date(args[i].$date);
            }
          }
        }
      }
      ...
      if (Array.isArray(filter[key])) {
        filter[key] = filter[key].map((subFilter: any) =>
          fixDateExpressions(subFilter)
        );
      }
    } else {
      // Recursively handle nested objects
      filter[key] = fixDateExpressions(filter[key]);
    }
  }
